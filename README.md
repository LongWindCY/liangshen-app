...
* `public/indexHome.html` is the page template;
* `src/indexHome.jse.js` is the JavaScript entry point.
...
Only files inside `public` can be used from `public/indexHome.html`.<br>
...
If you are concerned about using Webpack-specific semantics, you can put all your CSS right into `src/indexHome.css`. It would still be imported from `src/indexHomeHome.js`, but you could always remove that import if you later migrate to a different build tool.
...
Inside `indexHome.html`, you can use it like this:
...
Import Bootstrap CSS and optionally Bootstrap theme CSS in the beginning of your ```src/indexHome.jse.js``` file:
...
You can also access the environment variables starting with `REACT_APP_` in the `public/indexHome.html`. For example:
...
```
/             - static server returns indexHome.html with React app
/todos        - static server returns indexHome.html with React app
...
Then, on the server, regardless of the backend you use, you can read `indexHome.html` into memory and replace `__OG_TITLE__`, `__OG_DESCRIPTION__`, and any other placeholders with values depending on the current URL. Just make sure to sanitize and escape the interpolated values so that they are safe to embed into HTML!
...
* [`mount()`](http://airbnb.io/enzyme/docs/api/mount.html) in [Enzyme](http://airbnb.io/enzyme/indexHome.html)

In contrast, **jsdom is not needed** for the following APIs:

* [`TestUtils.createRenderer()`](https://facebook.github.io/react/docs/test-utils.html#shallow-rendering) (shallow rendering)
* [`shallow()`](http://airbnb.io/enzyme/docs/api/shallow.html) in [Enzyme](http://airbnb.io/enzyme/indexHome.html)
...
from [`src/indexHomeHome.js`](src/indexHomeHome.js).

If you had previously enabled service workers in your production deployment and
have decided that you would like to disable them for all your existing users,
you can swap out the call to `registerServiceWorker()` in
[`src/indexHome.jse.js`](src/indexHome.jse.js) first by modifying the service worker import:
```javascript
...
`npm run build` creates a `build` directory with a production build of your app. Set up your favorite HTTP server so that a visitor to your site is served `indexHome.html`, and requests to static paths like `/static/js/main.<hash>.js` are served with the contents of the `/static/js/main.<hash>.js` file.
...
app.get('/', function (req, res) {
  res.sendFile(path.join(__dirname, 'build', 'indexHome.html'));
});
...
This is because when there is a fresh page load for a `/todos/42`, the server looks for the file `build/todos/42` and does not find it. The server needs to be configured to respond to a request to `/todos/42` by serving `indexHome.html`. For example, we can amend our Express example above to serve `indexHome.html` for any unknown paths:
...
```
    ...
    RewriteRule ^ indexHome.html [QSA,L]
```
...
`/todos/42`, by serving the cached copy of your `indexHome.html`. This
...
When users install your app to the homescreen of their device the default configuration will make a shortcut to `/indexHome.html`. This may not work for client-side routers which expect the app to be served from `/`. Edit the web app manifest at [`public/manifest.json`](public/manifest.json) and change `start_url` to match the required URL scheme, for example:
...
This will make sure that all the asset paths are relative to `indexHome.html`. You will then be able to move your app from `http://mywebsite.com` to `http://mywebsite.com/relativepath` or even `http://mywebsite.com/relative/path` without having to rebuild it.
...
```sh
    ...
    ? Configure as a single-page app (rewrite all urls to /indexHome.html)? Yes
    ✔  Wrote build/indexHome.html
...
* Alternatively, you can use a trick to teach GitHub Pages to handle 404 by redirecting to your `indexHome.html` page with a special redirect parameter. You would need to add a `404.html` file with the redirection code to the `build` folder before deploying your project, and you’ll need to add code handling the redirect parameter to `indexHome.html`. You can find a detailed explanation of this technique [in this guide](https://github.com/rafrex/spa-github-pages).
...
Note that in order to support routers that use HTML5 `pushState` API, you may want to rename the `indexHome.html` in your build folder to `200.html` before deploying to Surge. This [ensures that every URL falls back to that file](https://surge.sh/help/adding-a-200-page-for-client-side-routing).
...
* If the watcher doesn’t see a file called `indexHome.jse.js` and you’re referencing it by the folder name, you [need to restart the watcher](https://github.com/facebookincubator/create-react-app/issues/1164) due to a Webpack bug.
